...
export const TransmittalList: React.FC<Props> = ({
  ...
  if (viewMode === 'cards') {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3.5 sm:gap-4 no-print">
        {forms.map((form) => {
          ...
          return (
            <div
              key={form.id}
              className="bg-white rounded-xl p-4 sm:p-5 border border-slate-200/80 hover:border-slate-300 hover:shadow-sm transition-all flex flex-col justify-between group"
            >
              <div>
                {/* Header: Form Number + Origin Tag + Status Toggle */}
  ...
  // Table View
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-2xs overflow-hidden no-print">
                ...
                <tr key={form.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="py-3 px-4 text-[11px] whitespace-nowrap">
                    <div className="flex items-center gap-1.5 text-slate-600">
                      ...
                      <span className="font-medium text-slate-800">{form.dropTo || form.receivedByName || '---'}</span>
                    </div>
                  </td>
